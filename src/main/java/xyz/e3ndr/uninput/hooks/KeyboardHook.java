...
public class KeyboardHook implements Closeable {
    private Uninput uninput;

    private Listener listener = new Listener();

    public KeyboardHook(Uninput uninput) {
    ...
    private class Listener implements NativeKeyListener {

        @Override
        public void nativeKeyPressed(NativeKeyEvent nativeEvent) {
            if (uninput.isMouseOnThisMachinesScreen()) return;

            int vk = conv(nativeEvent.getKeyCode());
    ...
    private static int conv(int vc) {
        // https://github.com/kwhat/jnativehook/blob/master/src/java/org/jnativehook/keyboard/SwingKeyAdapter.java#L77
